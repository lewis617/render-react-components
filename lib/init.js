...
const src2rrc = files =>
  files.map(comPath =>
    comPath
      .replace(/^src\//, '')
      .replace(/\//g, '_')
      .replace(/_index|\.jsx?/g, ''));

const init = (filter) => {
  glob('src/**/*.+(js|jsx|)', (err, res) => {
    if (err) {
...
import ${componentName} from '../${filteredFiles[i].replace(/.jsx?/, '')}';
...
const renderComponent = () => render(<${componentName}Contaner />, document.querySelector('#app'));

renderComponent();
if (module.hot) { module.hot.accept(renderComponent); }\n`,
          );
        }
      });
    }
